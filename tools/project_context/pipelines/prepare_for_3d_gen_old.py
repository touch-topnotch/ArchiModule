...
class PrepareFor3dGen(FormWindow):
    ...
    def _setup_select_sketch_view(self):
        ...
        self.prompt_edit.textChanged.connect(
             ...
             lambda: exporting.save_prop("prompt", self.prompt_edit.toPlainText()) if self.prompt_edit else None  # Use toPlainText()
        )
    ...
    def _load_and_prepare_image(self, image_path: str) -> bool:
        ...
        try:
            ...
            self.original_pixmap = pixmap.scaled(target_width, target_height, Qt.AspectRatioMode.KeepAspectRatio, Qt.TransformationMode.SmoothTransformation)
            if self.original_pixmap.isNull():
                 FreeCAD.Console.PrintError("_load_and_prepare_image: Scaled pixmap is null.\n")
                 return False

            if self.blurred_pixmap is None:
                blurred_image = apply_blur_effect(self.original_pixmap.copy(), 10)
                self.blurred_pixmap = QPixmap.fromImage(blurred_image)
    ...
    def _handle_sketch_selection(self, index: int):
        ...
        if hasattr(selected_cell, 'image_path'):
            self.selected_render_path = selected_cell.image_path  # type: ignore[attr-defined]
        ...
        for i, cell in enumerate(self.selection_gallery.cells):
            if cell and hasattr(cell, 'label'):
                if i == index:
                    ...
                    cell.label.setStyleSheet("border: 3px solid rgba(0, 160, 200, 0.9); border-radius: 15px;")  # type: ignore[attr-defined]
                    self._apply_effects_to_cell(cell, blur=False, opacity=1.0)
                else:
                    # Apply deselected style (no border) and add effects
                     cell.label.setStyleSheet("border: 0px;")  # type: ignore[attr-defined]
    ...
    def _apply_effects_to_cell(self, cell: GalleryCell, blur: bool, opacity: float):
        ...
        label = cell.label  # type: ignore[attr-defined]
        if not label:
            return
    ...
    def _handle_image_click(self, pos: QPoint):
        ...
        if orig_x is None or orig_y is None: # Click outside image bounds
             return
        ...
        try:
            ...
            tool_color = Qt.GlobalColor.cyan if self.selected_tool == self.ToolType.PEN else Qt.GlobalColor.red
            pen = QPen(tool_color, 30, Qt.PenStyle.SolidLine, Qt.PenCapStyle.RoundCap, Qt.PenJoinStyle.RoundJoin)
            painter.setPen(pen)
            painter.setOpacity(0.5) # Semi-transparent drawing
            painter.drawPoint(QPoint(int(orig_x), int(orig_y))) # Draw on the display coordinates
            painter.end()

            self.image_display_label.setPixmap(self.current_pixmap)

            # Store coordinates for API call
            assert self.display_scale_factors is not None
    ...
    def _call_remove_background_api(self, rb_input: models.RemoveBackgroundInput, pen_points_used: list, erased_points_used: list):
        ...
        if not (self.auth_session.is_authenticated()):
            self.auth_session.auto_login(callback=lambda _: self._call_remove_background_api(rb_input, pen_points_used, erased_points_used))
            return
    ...
    def _on_background_removed(self, result: Optional[models.RemoveBackgroundOutput], error: Optional[Exception], pen_points_used: list, erased_points_used: list):
        ...
        try:
            ...
            if not project_path:
                QMessageBox.warning(self, "Ошибка", "Не удалось определить путь к проекту.")
                return
            output_dir = os.path.join(project_path, "background_removed")
        ...
        except (IOError, binascii.Error, ValueError, Exception) as e:
    ...
    def _update_display_pixmap(self, new_foreground_pixmap: QPixmap):
         ...
         if self.blurred_pixmap and not new_foreground_pixmap.isNull():
             try:
                ...
                blurred_image = self.blurred_pixmap.toImage()
